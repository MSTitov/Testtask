...
local EMPTY_CELL = ' '      -- Занчение для обнуления ячеек

--[[
    Создание подтаблицы без указанного элемента. Используется при заполнении таблицы, 
    что бы при случайном перемешивании в таблице не получались готовые комбинации
--]]
...
--[[
    Перемешивание таблицы случайными кристалами, исключается 
    возможность готовых комбинаций при перемешивании
--]]
function matrix.mix()
  for i=1,TABLE_SIZE do
    ...
    for j=1,TABLE_SIZE do
      ...
      if i>2 then
        if (matrix[i-1][j] == newCristall and matrix[i-2][j] == newCristall) then
      ...
      if j>2 then
        if (matrix[i][j-1] == newCristall and matrix[i][j-2] == newCristall) then
          cristalsCount = cristalsCount - 1
          subCristalsTable = subTable(subCristalsTable, index)
          index = math.random(cristalsCount)
          newCristall = subCristalsTable[index]
...
--[[
    Перемещение двух кристолов местами
--]]
...
--[[
    Проверка горизонтальных комбинаций у кристалла, в случае наличия комбинаций
    они записываются в глобальную переменную
--]]
...
--[[
    Проверка вертикальных комбинаций у кристалла, в случае наличия комбинаций
    они записываются в глобальную переменную
--]]
...
--[[
    Проверка точки на наличие вертикальных и/или горизонтальных комбинаций. 
--]]
...
--[[
    Проверка, таблицы на отсутствие возможных ходов
--]]
...
--[[ 
    Вызов проверки на наличие возможных ходов и перемешивание таблицы при необходимости
--]]
function matrix.checkPossibleMove()
  if isHavePossibleMoves() then
    matrix.mix()
    _G['m'] = "Cristals mixed. No possible move"
  end
end

--[[ 
    При совпадении 3х кристоллов в ряд, из глобальной переменной забираются те координаты, 
    которые совпали, плюсуется кол-во очков в счёт, а значения заполняются пробелом для наглядности.
    Очки плюсуются с учётом комбинаций
--]]
local function clearMatchedLines()
  if #_G['matched'] < 1 then return false end
  
  for key, val in pairs(_G['matched']) do
    matrix[val.y][val.x] = EMPTY_CELL
  end
...
--[[
    Проверка двух координат на наличие комбинаций и вызов функции подсчёта полученных очков
--]]
...
--[[
    Функция проверяет есть ли в матрице возможные комбинации для удаления
--]]
...
--[[
    Поиск в матрице возможных комбинаций. После ручного пермещения кристалла и совпадения комбинации,
    имеющиеся выше кристаллы будут падать вниз и образовывать новые комбинации. Функция будет искать 
    эти комбинации и удалять их, давая за это очки
--]]
